...
updateSnake ( game, cmd ) =
    ...
    in
        if game.isDead then
            ...
            ( { game | snake = newHead :: newTail }, cmd )
...
initCmds =
    ...
    Task.perform SizeUpdated Window.getSize
...
toArrowChanged : Keyboard.RawKey -> Msg
toArrowChanged value =
    case (Keyboard.rawValue value) of
        "Space" ->
            ArrowPressed Space
...
renderBackground =
    rect [ x "0", y "0", width sizeDefault, height sizeDefault, backgroundColor ] []
